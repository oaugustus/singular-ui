# Gravity Elements — Plano de Implementação por Etapas

Complementa `gravity-elements-especificacao-tecnica.md`. Cobre todos os ~125 componentes do Nuxt UI, organizados em etapas sequenciais, cada uma com entregáveis e critério de aceite.
...
- `gravity-elements.module.js` (módulo raiz) e `core.module.js` — só wiring, sem lógica, agregando `gravityElements.core` e (posteriormente) `gravityElements.components`.
- `geTv` implementado e testado (fixtures comparando com `tailwind-variants`).
- `ngAria` e `ngAnimate` (módulos oficiais do AngularJS) incluídos como dependências do módulo `gravityElements.core`.
- Serviço `geOverlayStack` (glue code — pilha de overlays e roteamento do ESC).
- Serviço `geId` (geração de ids únicos para atributos ARIA, usado por praticamente todo componente de formulário/overlay a partir da Etapa 2).
- Serviço `geColorMode` (troca de tema claro/escuro, usado pelo componente `Theme` na Etapa 1).
- Diretivas wrapper finas: `ge-floating-position` (sobre Floating UI), `ge-focus-trap` (sobre focus-trap), `ge-hotkey` (sobre Mousetrap).
...
Critério de aceite: um componente "hello world" (`suBadge`, o mais simples) consegue ser montado end-to-end usando `geTv` + estrutura de módulos, com teste unitário passando; um segundo componente de teste usando `ge-floating-position` + `ge-hotkey` (ex.: um Tooltip simples) confirma que os wrappers de terceiros funcionam antes de avançar para as etapas seguintes.
...
Por quê primeiro: majoritariamente estáticos ou com estado local simples (Collapsible, Calendar são os únicos com lógica não trivial) — valida o padrão `component + theme + geTv` em volume antes dos casos complexos de formulário/overlay.
...
CommandPalette reaproveita `ge-floating-position` + `geOverlayStack` da Etapa 0, `ge-hotkey` (Mousetrap) para o atalho global, e **Fuse.js** para fuzzy search — as três peças já vêm prontas da fundação, então o trabalho aqui é montar o componente visual e a integração, não a lógica de busca/atalho.
...
Com `ge-focus-trap` (focus-trap), `ge-floating-position` (Floating UI) e `ge-hotkey` (Mousetrap para ESC) já prontos da Etapa 0, o trabalho desta etapa se concentra em: portal (`$compile` + `document.body`), bloqueio de scroll do body, empilhamento correto de múltiplos overlays via `geOverlayStack`, e transições de entrada/saída via `ngAnimate`. Não há mais lógica de foco/posicionamento própria para escrever — é a etapa que mais se beneficia da mudança de estratégia (era a de maior risco no plano anterior).

Critério de aceite: dois modais abertos simultaneamente (um sobre o outro) fecham na ordem correta com ESC (roteado pelo `geOverlayStack`); leitor de tela anuncia abertura/fechamento (via `ngAria`); foco retorna ao elemento que abriu o overlay ao fechar (garantido pelo focus-trap).
...
Critério de aceite: um shell de dashboard completo no demo app (sidebar colapsável + navbar + toolbar + busca) montado 100% com componentes `ge*`, sem HTML/CSS ad-hoc fora do sistema de tema.
...
Mantido aqui só como referência para uma eventual v2, caso o Gravity Elements precise cobrir landing pages/marketing além de painéis internos.
...
- Publicação como `gravity-elements` no npm (privado ou público, a definir) e do site de docs (GitHub Pages ou Vercel/Netlify, ambos com suporte nativo a Docusaurus).

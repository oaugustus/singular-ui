# Gravity Elements — Especificação Técnica
...
O Gravity Elements é uma biblioteca de componentes reutilizáveis para AngularJS 1.8.3, com paridade visual e de API com o Nuxt UI, e paridade estrutural de projeto com o guia de estilo do John Papa para AngularJS ([johnpapa/angular-styleguide, pasta `a1`](https://github.com/johnpapa/angular-styleguide/tree/main/a1) — confirmado como a referência exata deste projeto). Não é um site de blocos copiáveis: é um pacote instalável (`gravity-elements`), com componentes versionados, tema consistente e comportamento acessível, destinado a aplicações internas modernas — não a websites de marketing.
...
- Módulo declarado sem variável, via setter syntax, uma única vez por módulo (**Y021**, **Y023**); todo arquivo que adiciona algo ao módulo usa a getter syntax encadeada (`angular.module('gravityElements.element').component(...)`), nunca guarda o módulo numa variável (**Y022**).
...
- Prefixo curto e exclusivo para todo elemento/atributo customizado: `ge-` (ex.: `ge-button`, `ge-floating-position`), evitando colisão com `ng-` e com prefixos de outras libs (**Y073**, "Provide a Unique Directive Prefix").
...
│   ├── core/
│   │   ├── tv/
│   │   │   ├── tv.service.js            # geTv — resolvedor de variantes
│   │   │   └── tv.service.spec.js
│   │   ├── overlay/
...
│   │   ├── id/
│   │   │   └── id.service.js             # geId — geração de ids únicos p/ ARIA
│   │   └── core.module.js
│   ├── components/
│   │   ├── layout/
│   │   │   ├── app/
...
│   └── gravity-elements.module.js              # angular.module('gravityElements', ['gravityElements.core', 'gravityElements.components'])
...
Todo componente do Gravity Elements deve conter, no mínimo:

- `*.component.js` — definição do `.component()`, bindings documentados, controller nomeado, `controllerAs: 'vm'`.
- `*.html` — template.
- `*.theme.js` — objeto de tema (`slots`, `variants`, `compoundVariants`, `defaultVariants`), consumido via `geTv`.
...
(function () {
  ...
  angular.module('gravityElements.element').component('suButton', {
  ...
  ButtonController.$inject = ['geTv', 'suButtonTheme'];

  function ButtonController(geTv, suButtonTheme) {
    var vm = this;
    vm.$onInit = onInit;

    function onInit() {
      vm.classes = geTv(suButtonTheme)({
...
(function () {
  ...
  angular.module('gravityElements.element').constant('suButtonTheme', {
    slots: {
...
## 6. Especificação do `geTv` (motor de tema)
...
- **Testabilidade**: `geTv` deve ser testável isoladamente, sem Angular carregado, para permitir testes de fixture comparando a saída do Gravity Elements com a saída real do `tailwind-variants` para o mesmo input de props (ver seção 9).

## 7. Camada de interatividade (composição de bibliotecas prontas, decisão confirmada)

**Decisão:** a camada de interatividade não é escrita do zero. O Reka UI (exclusivo do Vue) é substituído por um conjunto de bibliotecas headless de terceiros, ativamente mantidas e agnósticas de framework, encapsuladas em diretivas/serviços finos do Gravity Elements. Regra geral do projeto: antes de escrever qualquer lógica de comportamento nova, verificar se um módulo oficial do AngularJS ou uma biblioteca vanilla já resolve o problema.

### 7.1 Módulos oficiais do próprio AngularJS (primeira escolha, sempre)

| Módulo | Uso no Gravity Elements |
|---|---|
...
Nenhuma dessas bibliotecas tem markup ou CSS próprio — todas retornam comportamento puro (posição calculada, foco gerenciado, elementos filtrados), deixando o Tailwind/`geTv` no controle total da aparência. Isso preserva a mesma divisão de responsabilidade do Reka UI original: comportamento desacoplado de estilo.

### 7.3 O que continua sendo código próprio do Gravity Elements (glue code, não reinvenção)

Só permanece custom aquilo que é integração específica com o ciclo de vida do Angular, sem equivalente pronto:

- **`geOverlayStack`**: serviço que mantém a pilha de overlays abertos, resolve z-index incrementalmente e decide qual overlay recebe o ESC do Mousetrap (o do topo, não todos). É orquestração, não reimplementação de foco/posicionamento.
- **Diretivas wrapper finas** (`ge-floating-position`, `ge-focus-trap`, `ge-hotkey`) que só chamam a API da respectiva biblioteca no `$onInit`/`$onDestroy` do componente Angular — poucas linhas cada, sem lógica de comportamento própria.
...
- Teste de fixture para `geTv`: gerar casos de teste a partir dos mesmos inputs de props usados nos testes do Nuxt UI original e comparar string de classes resultante (normalizada) — pega regressões de tema cedo.
...
O Nuxt UI é licenciado sob MIT, que permite uso, modificação e redistribuição — inclusive como base de um projeto derivado como o Gravity Elements — desde que o aviso de copyright original seja preservado. Ações concretas:
...
- `README.md` do Gravity Elements deixando claro que o projeto é um port independente, sem afiliação oficial com Nuxt Labs.
...
- Categoria **Color Mode** (`ColorModeAvatar/Button/Image/Select/Switch`) — variantes de outros componentes por tema claro/escuro; o `geColorMode` (serviço core) já cobre a troca de tema, essas variantes de componente ficam para v2.
